...
serve(async (req) => {
  ...
  try {
    ...
    if (timeEntryResponse.ok) {
      ...
      // Debug: Log first time entry to see exact field names from Dockmaster API
      if (timeEntries.length > 0) {
        console.log('FIRST TIME ENTRY FIELDS:', Object.keys(timeEntries[0]).join(', '))
        console.log('FIRST TIME ENTRY DATA:', JSON.stringify(timeEntries[0]))
      }

      // Group time entries by work order and opcode, track latest time
      // Use stopTime if available, otherwise fall back to startTime
      // Use a separator that won't appear in opcodes (|||)
      ...
      for (const entry of timeEntries) {
        ...
        // Prefer stopTime, fall back to startTime for open/active entries
        const timestamp = entry.stopTime || entry.startTime
        if (timestamp) {
